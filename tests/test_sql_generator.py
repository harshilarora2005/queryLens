...
def test_query_under_cap_executes():
    ...
    with (
        patch.object(bq_executor, "get_client", return_value=fake_client),
...
def test_query_over_cap_rejected_before_executing():
    ...
    real_calls = [
        call
        for call in fake_client.query.call_args_list
        if not (
            call.kwargs.get("job_config") and getattr(call.kwargs["job_config"], "dry_run", False)
        )
    ]
